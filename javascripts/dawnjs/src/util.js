  ...
  util = {
    ...
    uniqueConcat: function(arr, elem_arr) {
      ...
      for (_i = 0, _len = elem_arr.length; _i < _len; _i++) {
        i = elem_arr[_i];
        _results.push(util.uniquePush(arr, i));
      }
      return _results;
    },
    ...
    dump: function(type, file_key, obj) {
      ...
      if (util.indexedDBWrite) {
        return util.indexedDBWrite(type, {
          key: file_key + '.' + type,
          query: util.toObjString(obj)
        });
      } else {
        fs = require('fs');
        key = file_key;
    ...
    load: function(type, file_key, callback) {
      ...
      if (util.indexedDBRead) {
        return util.indexedDBRead(type, file_key, function(res) {
        ...
        if (fs.existsSync(file_name)) {
          ...
          return util.log('file not exist: ' + file_key);
        }
      }
    },
    ...
    fileWalk: function(root_dir, handler) {
      ...
      for (_i = 0, _len = ls.length; _i < _len; _i++) {
        ...
        if (fs.lstatSync(root_dir + name).isDirectory()) {
          _results.push(util.fileWalk(root_dir + name + '/', handler));
        } else {
  ...
  (function() {
    ...
    workDB = function(type, callback) {
      ...
      return req.onerror = function(e) {
        return util.log('IndexedDB Error: ' + e.target.errorCode);
      };
    };
    util.indexedDBRead = function(type, key, callback) {
    ...
    return util.indexedDBWrite = function(type, obj, callback) {
      return workDB(type, function(os) {
        var add_os_req;
