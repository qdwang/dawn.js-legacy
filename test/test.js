...
(function() {
  var BNF, BNFGrammar, CodeGen, Flow, G, IR, LR1, LexParser, Lexer, MixMap, SymbolTable, SyntaxParser, SyntaxTable, Zipper, args, ast, bnf, cache_dir, cb, dawnjs_dir, flow, fs, grammar, home, i, js_grammar, len, lex_syntax, localService, log, lp, lr1_parser_ast, lr1_parser_lex, lr1_parser_mm, ls, ls_ast, ls_lex, ls_mm, mm, parser, plain_tree, prepare_syntax, python_grammar, sampleA, sampleAST, sampleB, sampleC, sampleD, sampleLex, sample_ast_file, sample_obj, script, state, stringEqual, table, tables, test_script, test_syntax, util, zipperNodeTest;
  ...
  state = new SyntaxParser([['a', 1], ['b', 2], 'End', ['a', 3], ['b', 4], ['a', 5], 'End', ['c', 6], ['a', 7], ['b', 8], 'End', ['c', 9], ['e', 10], ['t', 11], 'End', ';']);
  ...
  zipperNodeTest = function() {
    var selectedA, selectedAb, selectedB, selectedBb, selectedValBb, selectedValBbb, selectedValbb;
    ...
    return log(selectedValBbb[0] === selectedValbb[0], 'Zipper Value Select 3');
  };
  ...
  tables = SymbolTable.walkGenerate(sampleAST, {
    Receiver: (function(x) {
      return x;
    }),
    Giver: null
  ...
  parser = function(fp, content) {
    var args, flow, getObjectName, grammar, lex_syntax, markers;
    lex_syntax = {
      'String': /'.*?[^\\]'|".*?[^\\]"/,
      'Dot': /\./,
      'Func': /function/,
    ...
    return [
      {
        type: 'ast',
        value: flow.result('ast')
      }, {
        type: 'lex',
        value: flow.result('lex_list')
      }, {
        type: 'mm',
        value: flow.result('mix_map')
      }
    ];
  };
  ...
  cb = function() {
    if (ls_mm && ls_ast && ls_lex) {
      ls_mm = MixMap.rebuild(ls_mm);
      LexParser.rebuild(ls_lex, ls_mm);
      SyntaxParser.rebuild(ls_ast, ls_mm);
      return log(ls_ast.leaves[0].leaves[2].leaves[0].leaves[9] === ls_mm.get(ls_lex[21], 'SyntaxNode'), 'Lex AST Load From Local Service');
    }
  };

  lr1_parser_lex = ls.get(__dirname + '/../src/LR1-parser.js', 'lex', function(res) {
    ls_lex = res;
    return cb();
  });

  lr1_parser_ast = ls.get(__dirname + '/../src/LR1-parser.js', 'ast', function(res) {
    ls_ast = res;
    return cb();
  });

  lr1_parser_mm = ls.get(__dirname + '/../src/LR1-parser.js', 'mm', function(res) {
    ls_mm = res;
    return cb();
  });
  ...
  util.dump('ast', 'sample_ast', ast.syntax_tree);

  util.dump('lex', 'sample_lex', sampleLex);

  util.load('ast', 'sample_ast', function(res) {
    SyntaxParser.rebuild(res, mm);
    ast.syntax_tree = res;
    return util.load('lex', 'sample_lex', function(lex_res) {
  ...
  js_grammar = 'Function -> "function " Id "(" Args* "){"\nAssignment -> "var " Receiver " = " Giver ";"\nDedent -> "}"\n:Args -> Id (", " Id)*\n:Receiver -> Id\n:Giver -> Id';
