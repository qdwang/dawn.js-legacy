// Generated by CoffeeScript 1.8.0
(function() {
  var localService, log, util;

  if (typeof self === 'undefined') {
    util = require('./../util.js');
  } else {
    util = self.util;
  }

  localService = function(dir_path) {
    this.dir_path = dir_path;
    return this;
  };

  localService.prototype.generate = function(parser, only, reparse) {
    var _this;
    if (only == null) {
      only = [];
    }
    if (reparse == null) {
      reparse = false;
    }
    _this = this;
    return util.fileWalk(this.dir_path, function(file_path) {
      var content, fs, item, o, parse_result, _i, _j, _len, _len1, _results;
      for (_i = 0, _len = only.length; _i < _len; _i++) {
        o = only[_i];
        if (o !== file_path.slice(-3)) {
          return false;
        }
      }
      fs = require('fs');
      content = fs.readFileSync(file_path);
      parse_result = parser(file_path, content.toString());
      _results = [];
      for (_j = 0, _len1 = parse_result.length; _j < _len1; _j++) {
        item = parse_result[_j];
        if (!reparse && util.existLocalCache(item.type, encodeURIComponent(file_path))) {
          continue;
        }
        _results.push(util.dump(item.type, encodeURIComponent(file_path), item.value));
      }
      return _results;
    });
  };

  localService.prototype.get = function(file_path, type, callback) {
    return util.load(type, encodeURIComponent(file_path), callback);
  };

  module.exports.localService = localService;

  log = function() {};

  log = util.log;

}).call(this);

//# sourceMappingURL=local.js.map
