...
(function() {
  ...
  BNFGrammer = function(bnf_raw_content) {
    var grammer_lines, grammer_lines_arr;
    this.raw_bnf_grammer = bnf_raw_content;
    grammer_lines = bnf_raw_content.split('\n');
    grammer_lines_arr = this.grammerPrepare(grammer_lines);
    this.grammerPrepare(grammer_lines);
    this.dist = grammer_lines_arr[0][0];
    this.bnf_grammer_pairs = grammer_lines_arr;
    return this;
  };

  BNFGrammer.prototype.grammerPrepare = function(grammer_lines) {
    ...
    for (_i = 0, _len = grammer_lines.length; _i < _len; _i++) {
      line = grammer_lines[_i];
      production = [];
      line_arr = line.split('->');
  ...
  BNFGrammer.isOneOrMore = function(closure) {
    return closure.length > 1 && '+' === closure.slice(-1);
  };

  BNFGrammer.removeSpecialMark = function(closure) {
    return closure.replace(/[\[\]\(\)\*\+]+/g, '');
  };

  BNFGrammer.hasSpecialMark = function(closure) {
    return closure.match(/[\[\]\(\)\*\+]+/);
  };

  BNFGrammer.prototype.makePlainBNF = function(stop) {
    ...
    for (_k = 0, _len2 = additional_reprs.length; _k < _len2; _k++) {
      ...
      this.bnf_grammer_pairs.push(repr);
    }
    if (!stop) {
...
//# sourceMappingURL=BNF-parser.map
